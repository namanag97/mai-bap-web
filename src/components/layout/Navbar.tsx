  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'bg-surface-overlay backdrop-blur-lg border-b border-border-default shadow-xs'
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          {siteConfig.navLinks.map(l => (
            <Link
              key={l.href}
              href={l.href}
              className="text-[10px] font-mono uppercase tracking-widest text-ink-muted hover:text-ink-primary transition-colors py-1"
            >
              {l.label}
            </Link>
          ))}
          <div className="pt-3 border-t border-border-subtle flex flex-col gap-3">
            <button
              onClick={toggle}
              aria-label="Toggle theme"
              className="p-2 border border-border-default hover:border-border-strong transition-colors text-ink-muted hover:text-ink-primary self-start"
            >
              {theme === 'dark' ? <Sun size={14} /> : <Moon size={14} />}
            </button>
